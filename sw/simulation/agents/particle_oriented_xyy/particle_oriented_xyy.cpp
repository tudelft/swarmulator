#include "particle_oriented_xyy.h"
...
particle_oriented_xyy::particle_oriented_xyy(int i, vector<float> s, float tstep)
{
  state = s;
  ID = i;
  dt = tstep;
...
vector<float> particle_oriented_xyy::state_update(vector<float> s)
{
  // NED frame
  // x+ towards North
  // y+ towards East
  ...
  float dpsirate = 0.;
  if (!manual) {
    controller->get_velocity_command(ID, vx_des, vy_des, dpsirate); // Command comes out in the local frame
  } else {
    vx_des = manualx;
    vy_des = manualy;
...
void particle_oriented_xyy::animation()
{
  draw d;
